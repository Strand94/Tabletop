...
| #   | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Priority | SP  |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | --- |
| 4   | ✅ **FIXED.** ~~No admin user-management UI (create/list/manage users) — §4.1/§6.~~ Added `/api/users` (admin-gated: list/create/change-role/reset-password/delete, with last-admin & self-delete guards) and an admin-only **Users** section in Settings. See `docs/superpowers/specs/2026-07-05-admin-user-management-design.md`.                                                                                                                                                 | P1       | 5   |
...
| 6   | ✅ **FIXED.** ~~Can't sort collection by the user's game rating.~~ Added `myRating` to `gameQuerySchema.sort`; `listGames` sorts by the requesting user's per-game rating in memory (unrated last), and a "My rating" sort option is wired into the collection. (`avgSessionRating`-in-list remains a separate follow-up.) See `docs/superpowers/specs/2026-07-05-rating-sort-and-taxonomy-mgmt-design.md`.                                                                         | P2       | 5   |
| 7   | ✅ **FIXED.** ~~No category/location creation UI.~~ Added admin-only **Categories** and **Locations** management sections in Settings (list/add/delete), plus `DELETE /api/categories/:id` and `DELETE /api/locations/:id` (create endpoints already existed). See `docs/superpowers/specs/2026-07-05-rating-sort-and-taxonomy-mgmt-design.md`.                                                                                                                                     | P2       | 3   |
